...
public class UpdateProductDialog extends JDialog
{
    ...
    private UUID productIdUpdate;

    public UpdateProductDialog(UUID  productIdUpdate) {
    ...
    {
        ProductDTO product = productDao.findById(productIdUpdate);
        txtNameProduct.setText(product.name());
        txtPrice.setValue(product.price());
        txtStock.setValue(product.stock());
        cbCategory.setSelectedItem(product.category().name());
    ...
    private void configureComponents()
    {
        ...
        lbTitle = new JLabel("Actualizar Producto");
        lbTitle.putClientProperty(FlatClientProperties.STYLE, "" +
                "font:bold +10");
        mainPanel.add(lbTitle);

        description = new JLabel("¡Mejoremos los sabores!");
    ...
    private void registerEmployee()
    {
        ...
        if (productDao.isProductExist(productIdUpdate, name))
        {
            JOptionPane.showMessageDialog(
                    this,
        ...
        UUID id = productDao.update(productIdUpdate, product);
        if (id != null)
        {
            dispose();
